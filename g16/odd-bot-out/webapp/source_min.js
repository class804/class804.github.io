...
var Audio={};Audio.inited=false;Audio.ctx=null;Audio.timer=null;Audio.allowed=false;Audio.paused=false;Audio.postAllowed=[];Audio.init=function(samples,sampleRate){Audio.inited=true;Audio.samples=samples;Audio.sampleRate=sampleRate;Audio.bufferingDelay=50/1000;Audio.bufferDurationSecs=Audio.samples/Audio.sampleRate;Audio.nextPlayTime=0;Audio.numSimultaneouslyQueuedBuffers=5;Audio.paused=false;Audio.resume();}
Audio.deinit=function(){Audio.allowed=false;Audio.inited=false;}
Audio.allow=function(){Audio.allowed=true;for(let i=0;i<Audio.postAllowed.length;i++){Audio.postAllowed[i](Audio.ctx);}
Audio.postAllowed=[];document.removeEventListener('keydown',Audio.initCtx);document.removeEventListener('mousedown',Audio.initCtx);document.removeEventListener('touchend',Audio.initCtx);}
Audio.use=function(callback){if(Audio.allowed){callback(Audio.ctx);return;}
Audio.postAllowed.push(callback);}
...
var Storage={};Storage.VERSION=1;Storage.PATH_VERSION='com.martinmagni.oddbotout/version';Storage.PREFIX='com.martinmagni.oddbotout/data/';Storage.LEGACY_PREFIX='/';Storage.stringToHex=function(str){var hex='';for(var i=0;i<str.length;i++){hex+=str.charCodeAt(i).toString(16)+" ";}
return hex;}
...
if(old_version!==null)
return;const version=1;let len;try{len=localStorage.length;}catch(e){return;}
...
function mouseWheel(ev){ev.preventDefault();_mouse_scroll(ev.deltaX,ev.deltaY);}
...
function pokiSendLevelData(){navigator.sendBeacon('./patch/null.json?https://leveldata.poki.io/oddbotout-7BB6514C1DF0568A','c7eea996-4017-4e5a-8b19-7acc5cc56695');}
